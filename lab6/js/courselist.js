...
function showCourseList() {
  ...
  $.each(courseList, function (i, obj) {
    btn =
      "<td><div class='d-grid gap-2'><button class='btn btn-success btn-lg' onclick='regCourse(\"" +
      i +
      "\")'><i class='far fa-check-square'></i></button></div></td>";
    $("#course_list").append(
      "<tr><td>" +
        obj.code +
        "</td><td>" +
        ...
        new Date(obj.endDate).toLocaleDateString(lang) +
        "</td><td class='text-end'>" +
        new Intl.NumberFormat(lang, { style: "decimal" }).format(
          obj.fee[lang]
        ) +
        "</td>" +
        btn +
