var labels = {
  ...
  HEAD_UPCOMING_COURSES: {
    "vi-VN": `Các khóa học sắp khai giảng (Quý khách hàng vui lòng liên hệ <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Văn phòng công ty</button> để đăng ký)`,
    "en-US": `Upcoming courses (please contact <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" >the company office</button> for registration)`,
  },
  COPYRIGHT: {
...
function getLang() {
  ...
  return lang ? lang : "vi-VN";
}

console.log(getLang());

var spanList = document.querySelectorAll("span.multilang");

var lang = getLang();

spanList.forEach((obj) => {
  $("#" + obj.id)
    .html(labels[obj.id][lang])
    .attr("title", labels[obj.id][lang]);
});

var tb_cell_name = document.querySelectorAll("span.multilang_name");

tb_cell_name.forEach((obj) => {
  $("#" + obj.id)
    .html(labels[obj.id][lang].name)
    .attr("title", labels[obj.id][lang].name);
});

var st = document.querySelectorAll("span.multilang_date");

st.forEach((obj) => {
  $("#" + obj.id + ".multilang_date")
    .html(labels[obj.id][lang].startDate)
    .attr("title", labels[obj.id][lang].startDate);
});
